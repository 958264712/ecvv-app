...
<template>
	<view class="chat-wrap">
		<!-- 标题栏 -->
		<shopro-navbar :background="{
				backgroundImage: 'linear-gradient(45deg, #00c4ff, #00a1ff)',
				color: '#FFF'
			}" backIconColor="#fff" customBar="0">
			<template v-slot:content>
				厂家名称
			</template>
		</shopro-navbar>

		<scroll-view class="scroll-box" scroll-y="true" :scroll-with-animation="false" :enable-back-to-top="true"
			:show-scrollbar="false" :scroll-into-view="scrollInto">
			<view class="cu-chat u-p-l-10 u-p-r-10">
				<view class="time">2024年05月08日 09:16</view>
				<!-- 询价主题 -->
				<inquiryinfo-type-card type="chat" />

				<block v-for="(chat, index) in chatList" :key="index">
					<!-- 系统消息-->
					<view class="" v-if="chat.type !== 'system'">
						<view class="time">2024年05月08日 09:16</view>
						<view class="cu-item">
							<view v-if="chat.identify === 'user'" class="cu-avatar round"
								:style="'background-image:url(' + (chat.server_avatar ? chat.server_avatar : $IMG_URL + '/imgs/chat/default-chat-avatar.png') + ')'">
							</view>
							<view class="main">
								<!-- 消息 -->
								<view class="content " style="max-width: 480rpx;"
									:class="chat.identify !== 'user' ? 'bg-white' : ' bg-gradual-purple'"
									v-if="chat.type === 'text'">
									<rich-text :nodes="chat.msg"></rich-text>
								</view>
							</view>
						</view>
					</view>
		...
		<view class="cu-bar foot input u-flex-col">
			...
			<view class="cu-bar u-flex-1" style="width: 100%;">
				<view class="input-wrap u-flex-1 u-flex u-col-center u-m-r-20 u-p-l-20">
					<input class="input" v-model="msgText" placeholder-style="font-size:26rpx;color:#999"
						:placeholder="$t('chat.place')" type="textarea"  maxlength="5000" row="5" :confirm-type="$t('chat.send')" :focus="false" 
						cursor-spacing="10" @focus="InputFocus" @blur="InputBlur" @input="onInput" @confirm="onSend" />
					<view class="action u-p-10" @tap="selEmoji"><text class="iconfont icon-biaoqing text-grey"></text>
					</view>
				</view>
				<button class="u-reset-button send-btn shadow" @tap.stop="onSend">{{$t('chat.send')}}</button>
			</view>
			<!-- 表情栏 -->
				...
				<swiper class="emoji-swiper" :indicator-dots="true" circular indicator-active-color="#7063D2"
					...
					<swiper-item v-for="(memoji, index) in newEmojiList" :key="index">
						<view class="swiper-item u-flex u-col-center">
							<view class="emoji-img" v-for="(memo, mindex) in memoji" :key="memo.file"
								@tap="onEmoji(memo)">
								<image class="emoji-img"
									:src="`${EMOJI_BASE_URL}/assets/addons/shopro/img/emoji/${memo.file}`" mode="">
								</image>
							</view>
						</view>
					</swiper-item>
	...
	export default {
		...
		data() {
			return {
				...
				EMOJI_BASE_URL: "https://demo.shopro.top",
				navTitle: '连接中...', //标题栏
		...
		methods: {
			init() {
				let that = this;
				uni.request({
					url: 'https://demo.shopro.top/addons/shopro/chat.index/init', // 替换为目标API的URL
					method: 'GET', // 或者 'POST', 'PUT', 'DELETE' 等，根据接口要求选择
					header: {
						'Content-Type': 'application/json', // 根据接口要求设置，如果是POST请求且发送JSON数据，通常需要设置此header
						// 'Authorization': 'Bearer your_token' // 如有需要，添加认证信息，如Token
					},
					success: (res) => {
						if (res.statusCode === 200) {
							if (res.data.code == 1) {
								console.log(res.data.data)
								that.templateList = res.data.data.question;
								that.emojiList = res.data.data.emoji;
								that.newEmojiList = this.sortData(res.data.data.emoji, 30);
								// that.socket = new Socket({
								// 		ping: 20000,
								// 		is_ssl: res.data.data.config.system.is_ssl,
								// 		gateway_port: res.data.data.config.system.gateway_port,
								// 		ssl_type: res.data.data.config.system.ssl_type
								// 	},
								// 	msg => {
								// 		that.parseMsgStatus(msg.data.data); //监听消息
								// 	}
								// );
							}
						} else {
...
<style lang="scss">
	...
	.foot {
		...
		.send-btn {
			...
			width: 110rpx;
			line-height: 64rpx;
			background: linear-gradient(90deg, #00c4ff, #00a1ff);
			border-radius: 32rpx;
			color: #fff;
		}
