<template>
	...
	<view class="uni-navbar u-flex u-row-between">
		...
		<view class="uni-page-head-bd" style="font-weight: bold;text-align: center;">{{$t("user.mine-collection")}}
		</view>
	...
	<view class="u-m-t-88">
		...
		<view class="u-p-20">
			<view v-if="collectList.length > 0 && current === 0" v-for="(item,index) in collectList" :key="item.id">
				<!-- 日期可全选 -->
				<view>
			...
			<view v-if="collectCompanyList.length > 0 && current === 1" v-for="(item,index) in collectCompanyList"
				:key="item.id">
				<view>
					<checkbox :value="item.check" style="transform:scale(0.7)" v-if="show"></checkbox>
					{{item.time}}
	...
	<view class="fixed u-flex u-row-between u-p-10" v-if="show">
		...
		<button type="warn" class="u-m-r-20">{{$t("user.collection-uncollect")}}</button>
	</view>
</template>
<script>
	...
	export default {
		...
		data() {
			return {
				current: 0,
				show: false,
				// items: [this.$t("search.filter-produce")+`(${this.current})`,
				// this.$t("search.filter-shop")+'(0)'],
				collectList: [{
						id: 1,
						time: '4月16日',
						check: false,
				...
				options: [{
					text: this.$t("user.collection-uncollect"),
					style: {
		...
		computed: {
			items(){
				return [
				this.$t("search.filter-produce")+`(${this.collectList.length})`,	
				this.$t("search.filter-shop")+`(${this.collectCompanyList.length})`
				]
			}
		}
	};
