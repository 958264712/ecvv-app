...
export default class Request {
	...
	async request(options = {}) {
		...
		return new Promise((resolve, reject) => {
			...
			options.success = res => {
				console.log(res);
				let setCookieHeader = res.header['Set-Cookie'];
				if (setCookieHeader) {
					// 解析Set-Cookie，这里简化处理，实际情况可能需要正则表达式等进行解析
					let cookieValue = parseCookie(setCookieHeader); // 这里需要自己实现parseCookie函数来提取Cookie值
					uni.setStorageSync('.AspNetCore.Session', cookieValue); // 存储到本地
				}
